...
class GenMain(models.Model):
    ...
    def _get_cases(self):
        _cases = GenMain.objects.all().count()
        if self.case_number:
            return _cases
        else:
            return _cases + 1
    ...
    def save(self, *args, **kwargs):
        ...
        super(GenMain, self).save(*args, **kwargs)

    class Meta:
        db_table = 'ovc_gen_registration'
        verbose_name = 'General Care'
        verbose_name_plural = 'General Cares'
...
class GenEvents(models.Model):
    event_id = models.UUIDField(
        primary_key=True, default=uuid.uuid1, editable=False)
    care = models.ForeignKey(GenMain, on_delete=models.CASCADE)
...
class GenForms(models.Model):
    form_id = models.UUIDField(
        primary_key=True, default=uuid.uuid1, editable=False)
    event = models.ForeignKey(GenEvents, on_delete=models.CASCADE)
    ...
    class Meta:
        db_table = 'ovc_gen_form'
        verbose_name = 'General Form data'
        verbose_name_plural = 'General Forms data'
