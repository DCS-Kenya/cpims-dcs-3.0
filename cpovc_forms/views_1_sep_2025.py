...
def forms_registry(request):
    ...
    OA_List = ['CCOA', 'CSRG', 'CCDF', 'CTRF', 'CLFC', 'CDSA',
               'CSAB', 'CSDQ', 'CHCP', 'CSTC', 'CCEA']
    is_OA = False
    if cpims_id:
        resultsets, rs_sets = [], []
        form_type = 'FTPC'
        ...
        return render(
            ...
            {'form': form, 'resultsets': [resultsets],
             'vals': vals, 'form_type': form_type,
             'case_uid': str(case_ids[0][0]), 'is_OA': is_OA})
...
def placement(request, id):
    ...
    try:
        ...
        ou_types = ['TNAP', 'TNRH', 'TNRS', 'TNRR', 'TNRB']
        is_si = True if ou_type in ou_types else False
        ...
        return render(request, 'forms/new_placement.html',
                      {'form': form, 'cases': cases, 'person_id': id,
...
def case_info(request, case_id):
    ...
    try:
        check_fields = ['sex_id', 'case_reporter_id',
                        'case_category_id', 'online_abuse_id']
        tids = {0: "Case Details"}
        ...
        case_cat_ids, case_subcat_ids = [], []
        case_categories = OVCCaseCategory.objects.filter(case_id_id=case_id)
        for ccat in case_categories:
            case_cat_ids.append(ccat.pk)
            if ccat.case_category in CATS:
                tr_case = True
        # Sub categories
        case_sub_categories = OVCCaseSubCategory.objects.filter(
            case_category_id__in=case_cat_ids)
        print('CC', case_sub_categories)
