...
from .utils import write_document, write_register

MEDIA_ROOT = settings.MEDIA_ROOT
DOC_ROOT = settings.DOCUMENT_ROOT
STATIC_ROOT = settings.STATICFILES_DIRS[0]
DCS_HDS = settings.DCS
...
@login_required
def reports_download_old(request, file_name):
    """Generic method for downloading files."""
    try:
        file_folder = ''
        ...
        for path, dirs, files in os.walk(MEDIA_ROOT):
            for filename in files:
                ...
                if (len(fnames) == 5):
                    ...
                    fname = base64.urlsafe_b64encode(filename.encode('ascii'))
...
def reports_pivot(request):
    ...
    try:
        form = CaseLoad(request.user)
        return render(
            request, 'reports/pivot.html', {'form': form, 'dcs': DCS_HDS})
    except Exception as e:
        raise e
    else:
        pass
    ...
    try:
        ...
        if results:
            for res in results[0]:
                titles.append(res)
        print('RID', report_ovc_id, report_id, 'oops')
        ...
        print('OVC error getting raw data - %s' % (str(e)))
        datas = {'file_name': '', 'data': [],
                 'status': 9, 'message': message, 'xls': xls}
        return JsonResponse(datas, content_type='application/json',
                            safe=False)
...
@login_required
def get_docs(request, id=4):
    """Download without printing."""
    # fname = 'test.pdf'
    rid = int(id)
    datenow = datetime.now()
    tarehe = datenow.strftime("%Y%M%d")
    report_name = 'Case Load and Data Quality'
    f_uname = 'Org-Unit_%s' % (report_name.replace(' ', '-'))
    f_name = "%s_%s_%s" % (f_uname, tarehe, rid)
    fname = "%s.pdf" % (f_name)
    # create_document(buffer, pd)
    response = HttpResponse(content_type='application/pdf')
    response['Content-Disposition'] = 'attachment; filename="%s"' % (fname)
    # Params
    ou_id = int(request.session.get('ou_primary', 0))
    ou_name = str(request.session.get('ou_primary_name', 'DCS'))
    ...
    if rid <= 5:
        ...
        f_name = "%s_%s_%s" % (f_uname, tarehe, id)
